...
from .forms import UserRegistrationForm, UserLoginForm, UserProfileForm, UserProfile
from.models import UserAccountActivationKey, User
...
def user_account_activation(request, username, activation_key):
    if UserAccountActivationKey.objects.filter(user__username=username, key=activation_key).exists():
        User.objects.filter(username=username, account_activated=True)
